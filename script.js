        ...
        try {
            ...
            console.log("showBulkInput définie");
            ...
            function addBulkPlayers() {
                ...
                const dayNumber = parseInt(modal.dataset.dayNumber) || 1;
                const division = parseInt(modal.dataset.division) || 1;
            ...
            console.log("addBulkPlayers définie");
                ...
                for (let division = 1; division <= 3; division++) {
                    ...
                    if (players.length === 0) {
                        ...
                        container.innerHTML = players.map(player => 
                            `<div class="player-tag" onclick="alert('Détails de ${player}')">
                                ${player}
            ...
            function generateMatchesForDay(dayNumber) {
                console.log("generateMatchesForDay appelée");
                if (!dayNumber) dayNumber = championship.currentDay;
                alert(`Génération des matchs pour la Journée ${dayNumber} (fonction simplifiée pour test)`);
            }
            window.generateMatchesForDay = generateMatchesForDay;

            // FONCTIONS SIMPLIFIÉES POUR LES AUTRES BOUTONS
            window.updateRankings = () => alert("Classements (fonction test)");
            window.exportChampionship = () => alert("Export (fonction test)");
            window.showImportModal = () => alert("Import (fonction test)");
            window.clearAllData = () => {
                if (confirm("Tout vider ?")) {
                    championship.days[1] = { players: { 1: [], 2: [], 3: [] }, matches: { 1: [], 2: [], 3: [] } };
                    updatePlayersDisplay(1);
                    showNotification("Données vidées", "warning");
                }
            };
            window.addNewDay = () => alert("Ajouter journée (fonction test)");
            window.switchTab = (day) => alert(`Passer à J${day} (fonction test)`);
            window.switchToGeneralRanking = () => alert("Classement général (fonction test)");
...
window.addPlayer = addPlayer;
console.log("addPlayer assignée");
...
window.showBulkInput = showBulkInput;
console.log("showBulkInput assignée");
...
window.closeBulkModal = closeBulkModal;
console.log("closeBulkModal assignée");
...
window.addBulkPlayers = addBulkPlayers;
console.log("addBulkPlayers assignée");
...
// Fonction de débogage pour vérifier que toutes les fonctions sont définies
function checkFunctions() {
    const functions = [
        'showBulkInput', 'addPlayer', 'generateMatchesForDay', 
        'updateRankings', 'exportChampionship', 'showImportModal', 
        'clearAllData'
    ];
    
    functions.forEach(funcName => {
        if (typeof window[funcName] === 'function') {
            console.log(`✅ ${funcName} est définie`);
        } else {
            console.error(`❌ ${funcName} n'est pas définie`);
        }
    });
}
